function EEG=generateJumpsERP(EEG)
% EEG=generateJumpsERP(EEG)
%
% Authors: Marco Buiatti and Velu Kumaravel, CIMeC (University of Trento, Italy), 2021.

stdEEG=mean(std(EEG.data,0,2));
% add artifacts on the first third of the data only, mimicking the data in
% Parise & Csibra, 2012.
L=round(length(EEG.times)/3);
% L=length(EEG.times);
